# hindcast_t2m/processing/ukmo_sys603.py
...
class UKMOSystem603Processor(BaseProcessor):
    ...
    Monthly temperature statistic (2m temperature)
    ...
    NOTE:
      Avoid creating a full-sized temporary variable in the Dataset (would allocate ~tens of GiB).
    """

    name = "egrr_sys603"

    def can_handle(self, ds: xr.Dataset, cfg: HindcastConfig) -> bool:
        centre = str(cfg.originating_centre).lower().strip().replace("_", "-")
    ...
    def process_grib(self, ds: xr.Dataset, cfg: HindcastConfig) -> xr.Dataset:
        ...
        if has_fcmonth and has_indexing:
            return self._process_layout_forecastMonth(ds, cfg, vname)

        if has_time_step:
            return self._process_layout_time_step(ds, cfg, vname)
    ...
    # ------------------------------------------------------------------
    def _process_layout_forecastMonth(self, ds: xr.Dataset, cfg: HindcastConfig, vname: str) -> xr.Dataset:
        rename = {}
        ...
        # Kelvin -> Celsius WITHOUT creating a full-sized temporary var in ds
        ...
        for l in lead:
            ...
            if idx.size == 0:
                pieces.append(temp_c.isel(lead_raw=0) * np.nan)
            else:
                pieces.append(temp_c.isel(lead_raw=int(idx[0])))

        t_sel = xr.concat(pieces, dim=xr.DataArray(lead, dims="lead", name="lead"))

        # climatology over member + init_time
        if "member" in t_sel.dims:
            t_sel = t_sel.mean(dim=("member", "init_time"), skipna=True)
        else:
            t_sel = t_sel.mean(dim=("init_time",), skipna=True)

        cal_month = np.array([int(str(m).split("-")[1]) for m in target_ym[0]], dtype=int)

        out = xr.Dataset({str(cfg.out_var_name): t_sel})
        ...
        out[str(cfg.out_var_name)].attrs.update(
            {
                "units": "degC",
                "long_name": "2m air temperature (monthly statistic)",
                "source": "UKMO (egrr) system 603 monthly statistic converted to degC.",
            }
        )
        return out
    ...
    # ------------------------------------------------------------------
    def _process_layout_time_step(self, ds: xr.Dataset, cfg: HindcastConfig, vname: str) -> xr.Dataset:
        # Normalize ensemble dim name
        if "number" in ds.dims and "member" not in ds.dims:
            ds = ds.rename({"number": "member"})

        # Kelvin -> Celsius WITHOUT creating a full-sized temporary var in ds
        rate = (ds[vname] - 273.15).astype("float32", copy=False)
        ...
        # Rebase so min positive lead becomes 1
        ...
        da_out = xr.DataArray(
            ...
            attrs={
                "units": "degC",
                "long_name": "2m air temperature (monthly statistic)",
                "source": "UKMO (egrr) system 603 monthly statistic converted to degC (sparse-safe).",
            },
        )
