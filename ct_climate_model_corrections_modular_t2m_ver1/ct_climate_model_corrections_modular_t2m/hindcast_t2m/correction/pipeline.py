...
from ..netcdf_builder import create_nc
    ...
    def run(self) -> None:
        ...
        for doy_dir in doy_dirs:
            ...
            for fp in forecast_files:
                ...
                if "lat" in ds_corr.coords or "lat" in ds_corr.dims:
                    ds_corr = ds_corr.rename({"lat": "latitude"})
                if "lon" in ds_corr.coords or "lon" in ds_corr.dims:
                    ds_corr = ds_corr.rename({"lon": "longitude"})

                da = ds_corr[self.cfg.var_name]
                extra_dims = [d for d in da.dims if d not in ("time", "latitude", "longitude")]
                if extra_dims:
                    raise ValueError(f"Dims extras nao suportadas para escrita com ncBuilder: {extra_dims}")

                da = da.transpose("time", "latitude", "longitude")

                create_nc(
                    path_out=str(out_path),
                    lons=ds_corr["longitude"].values,
                    lats=ds_corr["latitude"].values,
                    times=ds_corr["time"].values,
                    data=da.values,
                    units="C",
                    variable_name=self.cfg.var_name,
                )
