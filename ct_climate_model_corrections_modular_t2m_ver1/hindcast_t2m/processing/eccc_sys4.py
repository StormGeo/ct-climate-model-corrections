...
class ECCCSys4Processor(BaseProcessor):
    """
    ECCC sys4 (centre=cwao/eccc, system=4)
    ...
    def can_handle(self, ds: xr.Dataset, cfg: HindcastConfig) -> bool:
        ...
        # compatível com eccc_sys4 e eccc_subseas_glo
        if not (prefix.startswith("eccc_sys4") or prefix.startswith("eccc")):
            return False
        ...
        if sys_cfg == "4" and centre_cfg in ("eccc", "cwao"):
            return True
        if sys_ds == "4" and centre_ds in ("cwao", "eccc"):
            return True
        return False
    ...
    def process_grib(self, ds: xr.Dataset, cfg: HindcastConfig) -> xr.Dataset:
        ...
        for dim in ("time", "step", "latitude", "longitude"):
            if dim not in ds.dims:
                raise RuntimeError(f"ECCC expected dim '{dim}'. Got dims={ds.dims}")

        if "valid_time" not in ds:
            raise RuntimeError("ECCC expected 'valid_time' coordinate in GRIB.")
        ...
        if tgt_month < 1 or tgt_month > 12:
            raise RuntimeError("ECCC sys4: target_month não definido. Defina no pipeline antes de processar.")
        ...
        if not np.any(keep):
            raise RuntimeError("ECCC sys4: nenhum lead 1..6 encontrado via valid_time/month.")
        ...
        for ti in range(ds.sizes["time"]):
            for l in range(1, 7):
                ...
                for si in cand:
                    ...
                    # pode vir como (member, lat, lon) ou (lat, lon)
                    if vals.ndim == 3:
